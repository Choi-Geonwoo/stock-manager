...
def init_db() -> None:
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        # 기존 stocks.db를 사용하는 경우에도 상세 이력 컬럼을 추가합니다.
        existing_columns = {
            row[1]
            for row in conn.execute("PRAGMA table_info(send_history)").fetchall()
        }
        migrations = {
            "exchange_rate": "ALTER TABLE send_history ADD COLUMN exchange_rate REAL",
            "stock_count": "ALTER TABLE send_history ADD COLUMN stock_count INTEGER NOT NULL DEFAULT 0",
            "detail_json": "ALTER TABLE send_history ADD COLUMN detail_json TEXT",
        }
...
def save_send_history(
    ...
    """텔레그램 발송 결과와 당시 조회된 종목별 상세 내역을 저장합니다."""
    init_db()
...
def delete_send_history(history_id: int) -> None:
    init_db()
    with sqlite3.connect(DB_PATH) as conn:
        conn.execute("DELETE FROM send_history WHERE id = ?", (history_id,))
        conn.commit()


def clear_send_history() -> None:
    init_db()
    with sqlite3.connect(DB_PATH) as conn:
...
def get_market_cap(stock: Stock) -> float:
    """Finnhub company profile에서 시가총액을 조회합니다. 반환 단위는 백만 단위입니다."""
    api_key = os.getenv("FINNHUB_API_KEY", "").strip()
    if not api_key:
        return 0.0
    ...
    for symbol in candidates:
        try:
            ...
            if response.status_code != 200:
                continue
            data = response.json()
            value = data.get("marketCapitalization")
...
class StockApp(tk.Tk):
    ...
    def _build_widgets(self):
        ...
        stocks_tab = tk.Frame(notebook, bg=self.BG)
        history_tab = tk.Frame(notebook, bg=self.BG)
        notebook.add(stocks_tab, text="  종목 / 현재가  ")
        notebook.add(history_tab, text="  발송 이력  ")
    ...
    def _build_history_tab(self, parent):
        ...
        ttk.Button(
            top,
            text="상세 내역",
            command=self.on_show_history_detail,
            style="Primary.TButton",
        ).pack(side="right", padx=(6, 0))
        ...
        history_columns = ("id", "sent_at", "type", "result", "message")
        ...
        for col in history_columns:
            self.history_tree.heading(col, text=history_headers[col])
            self.history_tree.column(
                col,
                width=history_widths[col],
                anchor="center" if col != "message" else "w",
                stretch=(col == "message"),
            )
        ...
        self.history_tree.bind(
            "<Double-1>",
            lambda _e: self.on_show_history_detail(),
        )
    ...
    def _send_report(self, send_type: str, details: Optional[list] = None):
        ...
        self.result_queue.put(("send_done", send_type, ok, msg))
        ...
        for row in load_send_history():
            ...
            self.history_tree.insert(
                ...
                values=(
                    ...
                    send_type,
                    result,
                    f"{detail_text} · {message}",
                ),
            )

    def on_show_history_detail(self):
        sel = self.history_tree.selection()
        if not sel:
            messagebox.showinfo(
                "안내",
                "상세 내역을 확인할 발송 이력을 선택해 주세요.",
            )
            return

        history_id = int(sel[0])
        row = load_send_history_detail(history_id)
        if not row:
            messagebox.showerror("오류", "발송 이력을 찾을 수 없습니다.")
            return
        ...
        try:
            details = json.loads(detail_json or "[]")
        except Exception:
            details = []

        dialog = tk.Toplevel(self)
        dialog.title(f"발송 상세 내역 · #{history_id}")
        dialog.geometry("820x560")
        dialog.minsize(700, 450)
        dialog.configure(bg=self.BG)
        dialog.transient(self)

        header = tk.Frame(dialog, bg=self.BG, padx=22, pady=18)
        header.pack(fill="x")

        tk.Label(
            header,
            text=f"발송 상세 내역  #{history_id}",
            font=("Malgun Gothic", 16, "bold"),
            bg=self.BG,
            fg=self.TEXT,
        ).pack(anchor="w")
        ...
        summary = (
            f"{sent_at}  ·  {send_type}  ·  {status_text}"
            f"  ·  {stock_count}종목"
        )
        if exchange_rate:
            summary += f"  ·  환율 {exchange_rate:,.2f}원"
        ...
        card.pack(fill="both", expand=True, padx=22, pady=(0, 18))

        columns = ("code", "name", "market", "price", "change", "pct", "dividend")
        tree = ttk.Treeview(
            card,
            columns=columns,
            show="headings",
        ...
        bottom = tk.Frame(dialog, bg=self.BG, padx=22, pady=(0, 18))
        bottom.pack(fill="x")

        tk.Label(
            bottom,
            text=f"전송 결과: {message}",
            font=("Malgun Gothic", 9),
            bg=self.BG,
            fg=self.SUCCESS if success else self.DANGER,
        ).pack(anchor="w")
        ...
        if report:
            ttk.Button(
                bottom,
                text="원문 보기",
                command=lambda: self._show_report_text(report),
                style="Soft.TButton",
            ).pack(side="right", padx=(0, 7), pady=(8, 0))

    def _show_report_text(self, report: str):
        dialog = tk.Toplevel(self)
        dialog.title("발송 메시지 원문")
    ...
    def on_delete_history(self):
        ...
        delete_send_history(history_id)
        self._reload_history_table()
        self.status_var.set("발송 이력이 삭제되었습니다.")

    def on_clear_history(self):
        if not load_send_history():
            messagebox.showinfo(
                "안내",
                "삭제할 발송 이력이 없습니다.",
            )
        ...
        clear_send_history()
        self._reload_history_table()
        self.status_var.set("전체 발송 이력이 삭제되었습니다.")
        ...
        try:
            while True:
                ...
                if item[0] == "fetch_done":
                    ...
                    _, send_type, ok, msg = item
                    self.busy = False

                    if ok:
                        self.status_var.set(
                            f"{send_type} 발송 완료 · {datetime.now(KST).strftime('%H:%M:%S')}"
                        )
                    else:
                        self.status_var.set(
                            f"{send_type} 발송 실패"
                    ...
                    # 스케줄에 의한 자동 발송이 성공하면 발송 이력을 엑셀로 저장합니다.
                    if ok and "자동" in send_type:
                        self._export_latest_history_to_excel()
    ...
    def _export_latest_history_to_excel(self):
        """가장 최근 발송 이력을 엑셀로 저장합니다(자동 발송 시 호출)."""
        try:
            rows = load_send_history(limit=1)
            if not rows:
                return
            latest_id = rows[0][0]
            filepath = export_send_history_to_excel(latest_id, EXPORT_DIR)
            self.status_var.set(
                self.status_var.get() + f" · 엑셀 저장됨: {os.path.basename(filepath)}"
            )
        except ImportError:
            messagebox.showwarning(
                "엑셀 저장 실패",
                "엑셀 파일을 생성하려면 openpyxl 패키지가 필요합니다.\n"
                "터미널에서 'pip install openpyxl' 실행 후 다시 시도해 주세요.",
            )
        except Exception as e:
            messagebox.showerror(
                "엑셀 저장 오류",
                f"발송 이력을 엑셀로 저장하는 중 오류가 발생했습니다.\n{e}",
            )
